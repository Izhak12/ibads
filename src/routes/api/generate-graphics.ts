...
חלוקת אחריות של הקלט (קריטי):
בריף הלקוח הוא מקור העובדות והמסרים — שאב ממנו את הקופי, ה-USPs וזוויות המסר. את ההחלטות הוויזואליות קבע בעצמך ב-designBrief, בהתאם לצבעי המותג ולסגנון הרפרנסים. אל תמציא עובדות שלא מופיעות בבריף (מנות, שירותים, מחירים, ביקורות).

המשימה: לייצר ${amountOfGraphics} קונספטים מובחנים לחלוטין לגרפיקה מרובעת 1:1 עבור:
שם העסק: ${clientName}
בריף שיווקי (עובדות, USPs, זוויות מסר, קהל, טון): ${clientBrief}
צבעי מותג: ${brandColors}
טקסט חובה על הגרפיקה (אם צוין): ${optionalText}

לכל קונספט החזר 4 שדות:
1. headline (עברית) – כותרת קצרה, חדה, שיווקית, בלי שגיאות. נגזרת מזווית המסר שבחרת לקונספט הזה.
...
4. designBrief (עברית, 5–8 שורות) – בריף ארט־דירקטור מלא לקונספט הזה בלבד. חובה שיפרט:
   ...
   • שורת 3–4 USPs עם אייקון + לייבל קצר. הלייבלים חייבים להילקח מרשימת ה-USPs שבבריף הלקוח — אל תמציא חדשים.
...
חוקים:
- כל קונספט חייב לבחור זווית מסר שונה מרשימת "זוויות מסר" שבבריף (רגשי / FOMO / social proof / פרימיום / פרקטי וכו'). ציין בתחילת ה-designBrief באיזו זווית בחרת.
- שורת ה-USPs בתוך designBrief חייבת להשתמש בלייבלים מרשימת ה-USPs של הבריף בלבד, לא להמציא חדשים.
...
- אסור להמציא שם עסק, לוגו, מנות, שירותים, מחירים או ביקורות.
