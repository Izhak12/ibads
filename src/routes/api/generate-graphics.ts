...
הנחיות ל-CTA (הנעה לפעולה) — חובה:
- ה-CTA מתאים לשלב מוקדם במשפך (מודעה ראשונה, לא סגירת עסקה): רמת מחויבות נמוכה — קבלת מידע, הצעת מחיר, שיחת ייעוץ. אסור "קבעו פגישה" או כל דבר שדורש התחייבות זמן.
- העדף לשון רבים-סתמית ("קבלו", "גלו", "דברו איתנו") או שם-פעולה ("לקבלת הצעת מחיר", "לשיחת ייעוץ") — לא ציווי-יחיד ("גלה", "צור").
- אסור מילים בירוקרטיות או קרות ("לבירורים", "לפרטים נוספים בלבד"). ה-CTA צריך להרגיש כמו הזמנה.
- אסור דחיפות מלאכותית ("היום בלבד", "מבצע לזמן מוגבל") אלא אם הבריף מבקש זאת במפורש.
- שמור על טון וניסוח CTA עקביים בין כל הקונספטים של אותו לקוח.

בדיקת איות עצמית (חובה לפני החזרת התשובה):
עבור מילה-מילה על כל טקסט עברי שכתבת וּוודא שכל מילה תקנית וקיימת בעברית. אם יש ספק באיות — בחר ניסוח חלופי פשוט ובטוח. אסור להשתמש במילה שאינה קיימת בעברית תקנית (למשל "הרמוגיה" במקום "הרמוניה").

איסור על קופי גבוה-ריק (חובה):
אסור להשתמש בביטויים כלליים-נשגבים שלא אומרים שום דבר קונקרטי על סוג השירות או המוצר. דוגמאות לביטויים אסורים (דוגמית לסוג הבעיה, לא רשימה סגורה): "אוכל מרהיב ללא מאמץ", "טעם שידברו עליו", "טעם שידברו עלייך", "עושים את הקסם", "חוויה שאין כמותה" בלי שום פירוט נלווה.
כל כותרת ותת-כותרת חייבות יחד להעביר משמעות קונקרטית: איזה סוג שירות/מוצר זה, לא רק תחושה מופשטת. אם אתה קורא את הכותרת+תת-כותרת ולא יכול לענות בוודאות "זה [X סוג שירות]" — זה כישלון של הקופי, גם אם הוא נשמע "יפה".
...
הנחיות טיפוגרפיה שיש לכתוב בתוך designBrief (חובה):
- אסור פונטים דקורטיביים/קליגרפיים/"אמנותיים" לעברית — הם מעוותים אותיות (מ, ח, ע, צ) ופוגעים בקריאות.
- לכותרות: רק סריף או סאנס-סריף נקי, מודרני ותקני לעברית, מסוג שנפוץ בעיצוב פרימיום אמיתי.
- לגוף הטקסט ולתת-כותרות: אך ורק סאנס-סריף נקי וקריא.
- היררכיה גם במשקל ולא רק בגודל: כותרת במשקל בינוני-כבד (לא הכבד ביותר), תת-כותרת קלילה יותר וקטנה בבירור, CTA במשקל בולט אך בגודל קטן מהכותרת.
