...
1. כותרת חזקה וברורה (הגדולה ביותר, מודגשת): '${input.headline}'
   ...
   - כותרת עם אופי: כותרת טובה לא רק מתארת את השירות — היא יוצרת סקרנות, קול, או תחושת "כולם מדברים על זה". שאב מסגנונות כמו: משפט שנשמע כמו ציטוט/מוניטין ("שלא מפסיקים לדבר עליו"), ניגוד קצר וממוקד, או הבטחה נועזת אחת. הימנע מכותרות שטוחות שרק מתארות במילים ניטרליות מה שהתמונה כבר מראה.
...
מבנה תוכן מורחב — שכבות אופציונליות מעבר לכותרת/תת-כותרת/CTA:
בהתאם לבריף העסק ולמידע שסופק למעלה, שקול להוסיף עד שתי שכבות תוכן נוספות מתוך השלוש הבאות, ורק אם יש להן על מה לשבת. אל תמציא פרטים שלא סופקו:

א. פסקת גוף קצרה עם פירוט קונקרטי (2-4 שורות, פונט קטן מהתת-כותרת):
   אם סופקה רשימת מוצרים/שירותים קונקרטית של העסק — שלב אותה כאן בקצרה ובאופן מוחשי (למשל פירוט פריטים בתפריט, שירותים כלולים). זה מה שהופך "טעם מדהים" מופשט לתמונה קונקרטית שהגולש יכול לדמיין. אם לא סופק מידע קונקרטי — דלג על השכבה הזו.

ב. עוגן מידע/ערך (badge קטן, נפרד מהטקסט הראשי):
   אם סופק מחיר, טווח מחירים, או נתון קונקרטי אחר (כמות, זמן אספקה וכו') — הצג אותו כאלמנט גרפי נפרד ובולט (כמו "תג מחיר"), לא כטקסט רץ. עוגן קונקרטי מוריד חשש וחוסר-ודאות. דלג אם לא סופק נתון כזה.

ג. שורת אמון תחתונה (טראסט-בר) עם 2-4 אייקונים:
   אם סופקו יתרונות עובדתיים קצרים של העסק (הסמכה/תו תקן, זמינות, טווח שירות, ניסיון) — הצג אותם כשורה אחת בתחתית הגרפיקה, כל יתרון עם אייקון קטן לצידו. דלג אם אין יתרונות עובדתיים שסופקו.

עקרון מנחה לשכבות הנוספות: כל שכבת תוכן חדשה מתבססת אך ורק על מידע שסופק בבריף העסק. אל תמציא הסמכות, מחירים או פרטי מוצר שלא נמסרו — עדיף גרפיקה עם 3 שכבות טקסט נכונות מאשר 5 שכבות שחלקן בדויות. השכבות האלה אופציונליות ותלויות-מידע, לא חובה.

מלבד שלוש השכבות הראשיות והשכבות האופציונליות שנבחרו לפי הכללים למעלה — אין להוסיף טקסט נוסף.
...
- CTA דו-חלקי: כשזה מתאים לבריף, בנה את ה-CTA משני חלקים — הקשר/סיבה לפעולה + הפעולה עצמה (למשל "לשריון תאריך לאירוע הקרוב — דברו איתנו" / "לקבלת הצעת מחיר מותאמת — השאירו פרטים"). זה נותן לגולש סיבה ברורה לפעול עכשיו ולא רק פקודה מנותקת מהקשר.
...
כללי טיפוגרפיה לעברית (חובה):
- אסור להשתמש בפונטים דקורטיביים, מעוצבים-יתר, קליגרפיים, או "אמנותיים" לעברית — פונטים כאלה נוטים לעוות את צורת האותיות (בעיקר אותיות כמו מ, ח, ע, צ) ולפגוע בקריאות הבסיסית, גם אם הם נראים "יוקרתיים" בהסתכלות ראשונה.
- לכותרות: השתמש רק בגופנים סריפיים או סאנס-סריף נקיים, מודרניים ותקניים לעברית, כאלה שנפוצים בעיצוב פרימיום אמיתי (מגזינים, מותגי יוקרה) — לא בפונטים שנראים כמו "AI-דקורטיבי".
- מבחן קריאות: אם יש ולו ספק קל אם אות מסוימת תיקרא נכון על ידי דובר עברית ממוצע במבט חטוף (לא בקריאה מרוכזת) — זה פונט פסול, יש לבחור פונט נקי יותר.
- לגוף הטקסט ולתת-כותרות: אך ורק גופן סאנס-סריף נקי וקריא, ללא יוצא מן הכלל.

היררכיה במשקל (לא רק בגודל):
- כותרת: בולטת בגודל, אך במשקל בינוני-כבד (לא הכבד ביותר האפשרי) — כדי שהעין תוכל לסרוק אותה בקלות ולא תרגיש כמו קיר טקסט.
- תת-כותרת: משקל קליל יותר מהכותרת, גודל קטן יותר בבירור.
- CTA: יכול להיות במשקל בולט משלו (זה כפתור, לא טקסט זורם) אך בגודל קטן מהכותרת.
- העיקרון: ההיררכיה חייבת להיות ברורה גם למישהו שרק "סורק" את הגרפיקה במבט של חצי שנייה, לא רק למי שקורא אותה במלואה.

איסור על קופי גבוה-ריק (חובה):
- אסור להשתמש בביטויים כלליים-נשגבים שלא אומרים שום דבר קונקרטי על סוג השירות או המוצר. דוגמאות לביטויים אסורים (לא רשימה סגורה, אלא דוגמית לסוג הבעיה שיש להימנע ממנה): "אוכל מרהיב ללא מאמץ", "טעם שידברו עליו", "טעם שידברו עלייך", "עושים את הקסם", "חוויה שאין כמותה" בלי שום פירוט נלווה.
- כל כותרת ותת-כותרת חייבות יחד להעביר משמעות קונקרטית: איזה סוג שירות/מוצר זה, לא רק תחושה מופשטת. אם אתה קורא את הכותרת+תת-כותרת בעצמך ולא יכול לענות בוודאות "זה [X סוג שירות]" — זה כישלון של הקופי, גם אם הוא נשמע "יפה".
- בדוק את עצמך לפני סיום: קרא רק את הכותרת לבד (בלי תת-כותרת) — האם אדם זר שרואה רק אותה, בלי שום הקשר נוסף, יודע לפחות באיזה תחום כללי מדובר? אם לא — הכותרת חייבת לכלול רמז לתחום, לא להישאר מופשטת לגמרי.
...
- אסור פונט דקורטיבי/קליגרפי/"אמנותי" לעברית שמעוות אותיות או פוגע בקריאות.
- אסור היררכיה שמבוססת על גודל בלבד בלי הבדלי משקל בין הכותרת, התת-כותרת וה-CTA.
- אסור קופי גבוה-ריק שלא מבהיר מהו סוג השירות.
